    ...
    public partial class BackOficeResultManager : UserControl
    {
        ...
        private void btnModifiy_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (panelEventMenu.Visible)
                {
                    EventControler.Modify(Int32.Parse(txtEventID.Text), txtEventName.Text, txtEventStartDateYear.Text, txtEventEndDateYear.Text, Int32.Parse(txtStageNum.Text), Int32.Parse(txtTimeNumber.Text), txtTimeDescription.Text);
                    MessageBox.Show("Evento" + txtEventID.Text + "modificado");
                }
                else if (panelJudgeMenu.Visible)
                {
                    JudgeControler.Modify(Int32.Parse(txtJudgeId.Text), txtJudgeName.Text, txtJudgeLastName1.Text, txtJudgeLastName2.Text, txtJudgeCity.Text, txtJudgeState.Text, txtJudgeCountry.Text);
                    MessageBox.Show("Juez" + txtJudgeId.Text + "modificado");
                }
                else if (panelStageMenu.Visible)
                {
                    StageControler.Modify(Int32.Parse(txtStageId.Text), txtStageName.Text, txtStageCity.Text, txtStageStreet.Text, Int32.Parse(txtStageNum.Text), txtStageState.Text, txtStageCountry.Text);
                    MessageBox.Show("Estadio" + txtStageId.Text + "modificado");
                }
                else if (panelActionMenu.Visible)
                {
                    string Time = getTime();
                    ActionControler.Modify(Int32.Parse(txtActionId.Text), Int32.Parse(txtIdTeam.Text), Int32.Parse(txtIdPlayer.Text), Int32.Parse(txtIdTime.Text), Int32.Parse(txtActionQuantity.Text), txtActionType.Text, txtActionContext.Text, Time, txtActionCategory.Text);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                JudgeControler.Delete(Int32.Parse(txtJudgeId.Text));
                MessageBox.Show($"Juez {txtJudgeId.Text} Eliminado");
            }
            else if (panelStageMenu.Visible)
            {
                StageControler.Delete(Int32.Parse(txtStageId.Text));
                MessageBox.Show($"Estadio {txtStageId.Text} Eliminado");
            }
            else if (panelActionMenu.Visible)
            {
                StageControler.Delete(Int32.Parse(txtActionId.Text));
