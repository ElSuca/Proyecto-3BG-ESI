...
namespace NewAPIResult.Models
{
    public class EventIdModel : Model
    {
        ...
        {
            this.command.CommandText = $"SELECT ID FROM EVENT JOIN (EVENT_SPO) ON (EVENT.ID = EVENT_SPO.ID_EVENT) WHERE EVENT_SPO.ID_SPO=@S ORDER BY EVENT.STARTDATE LIMIT 5 OFFSET @P";
